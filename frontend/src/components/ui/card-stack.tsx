...
export function CardStack({
  ...
  const containerStyles = {
    ...
    grid: "grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl",
    list: "flex flex-col gap-4 max-w-lg",
  }
            ...
            {displayCards.map((card) => {
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    "cursor-pointer rounded-2xl border border-border/50 bg-background/80 backdrop-blur-sm p-5 shadow-sm",
                    "hover:border-primary/50 hover:shadow-md transition-all duration-300",
                    layout === "stack" && "absolute w-64 h-56",
                    layout === "stack" && isTopCard && "cursor-grab active:cursor-grabbing shadow-lg shadow-primary/5",
                    layout === "grid" && "w-full aspect-[4/3] flex flex-col",
                    layout === "list" && "w-full flex-col",
                    isExpanded && "ring-2 ring-primary bg-primary/5 shadow-md shadow-primary/10",
                  )}
                  style={{
                ...
                >
                  ...
                  {isTopCard && layout === "stack" && (
                    <div className="absolute bottom-3 left-0 right-0 text-center">
                      <span className="text-xs font-medium text-muted-foreground/60 uppercase tracking-wider">Swipe to navigate</span>
                    </div>
                  )}
                </motion.div>
              )
